...
static constexpr size_t PREFILTER_BITSET_BYTES = 64 * 1024;

using namespace std;

int main(int argc, char** argv) {
  if (argc < 3 || argc > 4) {
    cout << "Usage: " << argv[0] << " <XCLBIN> <packet_file> [prefilter.bitset]"
         << endl;
    cout << "  packet_file : binary file with N x 64-byte packets" << endl;
    cout << "  prefilter.bitset : optional raw 64KiB Bloom table generated by"
         << " sw/prefilter_builder" << endl;
    return EXIT_FAILURE;
  }

  string xclbin_file = argv[1];
  string packet_file = argv[2];
  string bitset_file = (argc == 4) ? argv[3] : "";
  ...
  if (!pf.read(reinterpret_cast<char*>(pkt_data.data()),
               static_cast<streamsize>(pkt_size))) {
    cerr << "ERROR: failed to read packet file: " << packet_file << endl;
    return EXIT_FAILURE;
  }
  pf.close();

  vector<uint8_t> bitset_data(PREFILTER_BITSET_BYTES, 0);
  if (!bitset_file.empty()) {
    ifstream bf(bitset_file, ios::binary | ios::ate);
    if (!bf.is_open()) {
      cerr << "ERROR: cannot open prefilter bitset: " << bitset_file << endl;
      return EXIT_FAILURE;
    }
    size_t bitset_size = bf.tellg();
    if (bitset_size != PREFILTER_BITSET_BYTES) {
      cerr << "ERROR: prefilter bitset must be exactly "
           << PREFILTER_BITSET_BYTES << " bytes, got " << bitset_size << endl;
      return EXIT_FAILURE;
    }
    bf.seekg(0);
    if (!bf.read(reinterpret_cast<char*>(bitset_data.data()),
                 static_cast<streamsize>(bitset_data.size()))) {
      cerr << "ERROR: failed to read prefilter bitset: " << bitset_file << endl;
      return EXIT_FAILURE;
    }
    bf.close();
  }
  ...
  cout << "  Packets: " << packet_file << " (" << num_packets << " x 64B)"
       << endl;
  if (!bitset_file.empty()) {
    cout << "  Prefilter bitset: " << bitset_file << " (64KiB)" << endl;
  } else {
    cout << "  Prefilter bitset: <all-zero table>" << endl;
  }

  // Buffer layout:
  //   Input  = 64KiB packed Bloom bitset + N x 64B packets
  //   Output = (N+1) x 64B words (header + N prefilter hit counts)
  size_t in_bytes = PREFILTER_BITSET_BYTES + num_packets * PACKET_BYTES;
  ...
  memcpy(in_map, bitset_data.data(), bitset_data.size());
  memcpy(in_map + PREFILTER_BITSET_BYTES, pkt_data.data(), pkt_size);
  ...
  uint32_t leTsPut, leTsFeed, leTsDrain, leTsFilterDone;
  uint32_t lePacketsAccepted, lePacketsCompleted, leValidAnchors;
  uint32_t lePreFilterLookupReqs, lePreFilterLookupResps, lePreFilterHits;
  uint32_t leFilterBusyCycles;
  ...
  memcpy(&leTsFilterDone, out_map + 32, 4);
  memcpy(&lePacketsAccepted, out_map + 36, 4);
  memcpy(&lePacketsCompleted, out_map + 40, 4);
  memcpy(&leValidAnchors, out_map + 44, 4);
  memcpy(&lePreFilterLookupReqs, out_map + 48, 4);
  memcpy(&lePreFilterLookupResps, out_map + 52, 4);
  memcpy(&lePreFilterHits, out_map + 56, 4);
  memcpy(&leFilterBusyCycles, out_map + 60, 4);
  ...
  cout << "  tsFilterDone  = " << leTsFilterDone << endl;
  cout << endl;
  ...
  cout << "  Prefilter lookup/count latency (drain -> filter done) = "
       << (leTsFilterDone - leTsDrain) << " cycles" << endl;
  cout << "  Total LongEngine latency (put -> filter done) = "
       << (leTsFilterDone - leTsPut) << " cycles" << endl;
  cout << endl;

  cout << "=== LongEngine Aggregate Counters ===" << endl;
  cout << "  packetsAccepted      = " << lePacketsAccepted << endl;
  cout << "  packetsCompleted     = " << lePacketsCompleted << endl;
  cout << "  validAnchors         = " << leValidAnchors << endl;
  cout << "  prefilterLookupReqs  = " << lePreFilterLookupReqs << endl;
  cout << "  prefilterLookupResps = " << lePreFilterLookupResps << endl;
  cout << "  prefilterHits        = " << lePreFilterHits << endl;
  cout << "  filterBusyCycles     = " << leFilterBusyCycles << endl;
  cout << endl;
  ...
  cout << "  Note: Bloom table was loaded from the input buffer prefix."
       << endl;
  ...
  if (num_packets > sample_count) {
    cout << "  ... " << (num_packets - sample_count) << " more packets omitted"
         << endl;
  }
